...
namespace Korp.Billing.Api.Middleware;
...
public class ExceptionHandlingMiddleware(RequestDelegate next, ILogger<ExceptionHandlingMiddleware> logger)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            logger.LogError(ex, "Erro não tratado no serviço de faturamento");
            await WriteAsync(context, StatusCodes.Status500InternalServerError, "Erro interno",
                "Falha inesperada no serviço de faturamento. Tente novamente.", "INTERNAL");
        }
    }
